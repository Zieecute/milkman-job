...
AddEventHandler('crossbite_milk:pickedUp', function()
	...
	if xItem.limit ~= 300 and (xItem.count + zItem) > xItem.limit then
		--
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Inventoryfull', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'success', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
	...
	if not playersProcessing[source] then
		...
		playersProcessing[_source] = ESX.SetTimeout(Config.WaitProcess*1000, function()
			...
			elseif milk.count < Config.Normal then
				--
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'empty', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
			else
				xPlayer.removeInventoryItem('milk', Config.Normal)
				xPlayer.addInventoryItem('milk_package', Config.Packgrage)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'success', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
			end
